...
class CarTestCase(TestCase):
    def setUp(self):
        ...
        user_alex = User.objects.create(
            name="Alex", surname="Petrov", favorite=favorite
        )
        Car.objects.create(
            brand="BMW", model="x5", slug="BMW-x5", year_produced=2000, seller=user_alex
        )
        Car.objects.create(
            brand="Toyota",
            model="Prius",
        ...
        for i in range(5):
            Car.objects.create(
                ...
                model=f"x5_{i}",
                slug=f"BMW-x5_{i}",
                guarantee_period=self.PERIOD[i],
                seller=user_alex,
            )
