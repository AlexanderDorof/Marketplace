...
from main_app.models import Car, ItemForCar, ItemForMotorcycle, Motocycle, Service, User
...
def insert_cars(num=20, filename="cars_mini"):
    photo_cars_path = rf"{BASE_DIR}/media/photos/cars/{filename}"
    ...
    for i in range(num):
        ...
        photo = rf"photos/cars/{filename}/{random.choice(photo_cars)}"
        ...
        Car.objects.create(
            ...
            price=price,
            photo=photo,
            seller_id=random.choice(users_id()),
        )


def insert_motors(num=20, filename="motors"):
    photo_motors = list(os.walk(rf"{BASE_DIR}/media/photos/motos/{filename}"))[0][2]
    ColorsType = (
        "red",
        "blue",
        "green",
    ...
    for i in range(num):
        ...
        used_car = random.choices((True, False), weights=[0.8, 0.2], k=1)[0]
        price = random.randint(5_000, 50_000)
        photo = rf"photos/motos/motors/{random.choice(photo_motors)}"
        slug = f"{brand}-{model}-{price}"
        slug = slugify(slug)
        Motocycle.objects.create(
            brand=brand,
            model=model,
            year_produced=year_produced,
            engine_power=engine_power,
...
def insert_services(num=5):
    for i in range(num):
        in_charge = random.choice(
            ("Смирнов И.И.", "Сидоров А.К.", "Петров Г.С.", "Анохин Е.З.")
        )
...
def insert_items_motors(num=5):
    ...
    for i in range(num):
        ...
        slug = slugify(slug)
        try:
            ItemForMotorcycle.objects.create(
                title=title,
                description=description,
                is_available=is_available,
                slug=slug,
