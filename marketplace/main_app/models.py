...
class Vehicle(models.Model):
    ...
    year_produced = models.PositiveSmallIntegerField(
        blank=True,
        default=2000,
    ...
    guarantee_period = models.SmallIntegerField(
        choices=PERIOD, default=12, verbose_name="Гарантийный период"
    )
    ...
    engine_power = models.PositiveSmallIntegerField(
        help_text="В лошадиных силах(л/с)",
        default=1600,
    ...
    price = models.PositiveSmallIntegerField(
        default=1000, help_text="В долларах ($)", verbose_name="Цена"
    )
...
class Car(Vehicle):
    ...
    slug = models.SlugField(
        max_length=150, unique=True, db_index=True, verbose_name="URL slug"
    )
    ...
    drive_type = models.CharField(
        choices=DRIVE_TYPE, max_length=15, default="Front", verbose_name="Тип привода"
    )
    ...
    seller = models.ForeignKey(
        "User", on_delete=models.CASCADE, verbose_name="Продавец"
    )
...
class Service(models.Model):
    ...
    def russian_to_english_transliteration(cls, russian_sentence):
        ...
        return "".join(
            translit_map.get(letter, letter) for letter in russian_sentence.lower()
        )
    ...
    motorcycles_service = models.ManyToManyField(
        Motocycle, blank=True, verbose_name="Подходит для:"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        self.guarantee = future_date
        if not self.slug:
            slug = Service.russian_to_english_transliteration(
                f"{self.title}-{self.price}-{self.in_charge}"
            )
            self.slug = slugify(slug)
        try:
            super().save(*args, **kwargs)
...
class User(models.Model):
    ...
    sold = models.PositiveSmallIntegerField(
        default=0, verbose_name="Продано автомобилей"
    )
    ...
    def __str__(self):
        return (
            f"{self.surname} {self.name} {self.second_name if self.second_name else ''}"
        )
...
class Favorite(models.Model):
    favorite_cars = models.ManyToManyField(
        Car, blank=True, verbose_name="Избранное автомобили"
    )
    favorite_moto = models.ManyToManyField(
        Motocycle, blank=True, verbose_name="Избранное мотоциклы"
    )
