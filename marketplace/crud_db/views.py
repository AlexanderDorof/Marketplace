...
MotosList = type(
    "MotosList", (VehicleList,), {"model": Motocycle, "title": "Каталог мотоциклов"}
)
ServicesList = type(
    "ServicesList",
    (VehicleList,),
...
AddMoto = type(
    "AddMoto", (AddItem,), {"form_class": AddMotoForm, "vehicle": "мотоцикл"}
)
...
CarEditView = type(
    "CarEditView", (ItemEditView,), {"model": Car, "success_url": reverse_lazy("cars")}
)
MotoEditView = type(
    "MotoEditView",
    (ItemEditView,),
...
class VehicleDeleteView(AuthorPermissionsMixin, DataMixin, DeleteView):
    ...
    context_object_name = "item"
    # URL-name, redirected after successful deletion
    success_url = reverse_lazy("cars")
    # Template for confirmation deletion
...
CarDeleteView = type(
    "CarDeleteView",
    (VehicleDeleteView,),
    {"model": Car, "success_url": reverse_lazy("cars")},
)
MotoDeleteView = type(
    "MotoDeleteView",
    (VehicleDeleteView,),
